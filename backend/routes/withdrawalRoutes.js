...
async function calculateBalance(sitterId) {
  ...
  // Get ALL completed, paid bookings regardless of hold date to calculate TOTAL earnings
  ...
  // Apply 10% commission deduction logic to match what is credited to sitter
  const totalEarnings = allBookings.reduce(
    (sum, b) => sum + ((b.servicePrice || b.payment.amount || 0) * 0.9),
    0
  );

  // Get ONLY completed, paid bookings that are past hold period to calculate CLEARED earnings
  const clearedBookings = await Booking.find({
    sitterId: sitterObjectId,
    status: "completed",
    "payment.paid": true,
  ...
  const clearedEarnings = clearedBookings.reduce(
    (sum, b) => sum + ((b.servicePrice || b.payment.amount || 0) * 0.9),
    0
  );
  ...
  // Available balance is based on CLEARED earnings
  const availableBalance = Math.max(0, clearedEarnings - withdrawnAmount);

  // Pending Clearance
  const pendingClearance = Math.max(0, totalEarnings - clearedEarnings);
